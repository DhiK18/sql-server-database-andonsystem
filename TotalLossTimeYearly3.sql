...
IF OBJECT_ID('dbo.TotalLossTimeYearly3', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.TotalLossTimeYearly3 (
    ...
    CREATE UNIQUE INDEX uq_TotalLossTimeYearly3
    ON dbo.TotalLossTimeYearly3(line, problem, periodStart);
END
GO

CREATE OR ALTER TRIGGER dbo.trg_UpdateTotalLossTimeYearly3
...
BEGIN
    ...
    MERGE dbo.TotalLossTimeYearly3 AS tgt
    USING (
        SELECT
            line,
            problem,
            year,
            periodStart,
            DATEADD(YEAR, 1, periodStart) AS periodEnd,
            SUM(durationSeconds) AS durationSeconds,
            SUM(durationSeconds) / 60.0 AS lossMinutes
        FROM Pairs
        GROUP BY line, problem, year, periodStart
    ) src
