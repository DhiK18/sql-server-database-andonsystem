...
IF OBJECT_ID('dbo.TotalLossTimeMonthly3', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.TotalLossTimeMonthly3 (
    ...
    CREATE UNIQUE INDEX uq_TotalLossTimeMonthly3
    ON dbo.TotalLossTimeMonthly3(line, problem, periodStart);
END
GO

CREATE OR ALTER TRIGGER dbo.trg_UpdateTotalLossTimeMonthly3
...
BEGIN
    ...
    MERGE dbo.TotalLossTimeMonthly3 AS tgt
    USING (
        SELECT
            line,
            problem,
            month,
            periodStart,
            DATEADD(MONTH, 1, periodStart) AS periodEnd,
            SUM(durationSeconds) AS durationSeconds,
            SUM(durationSeconds) / 60.0 AS lossMinutes
        FROM Pairs
        GROUP BY line, problem, month, periodStart
    ) src
